...
hs_t_matrix, x_axis, y_axis, quad_mesh  = plt.hist2d(hs_t_df["tm02"].values.tolist(), hs_t_df["hm0"].values.tolist(), 
                                                     [range(orcaflex_batch.x_bins+1), range(orcaflex_batch.y_bins+1)])    
...
damage_copper_export_pthies = ["Hs_sim", "T_sim", "dir_sim", "n_sim", "damage_copper_pthies", "scaled_damage_copper_pthies", 
                               "damage_copper_pthies_total"]
damage_copper_export_dbeier = ["Hs_sim", "T_sim", "dir_sim", "n_sim", "damage_copper_dbeier", "scaled_damage_copper_dbeier", 
                               "damage_copper_dbeier_total"]
damage_steel_export_dbeier = ["Hs_sim", "T_sim", "dir_sim", "n_sim", "damage_steel_dbeier", "scaled_damage_steel_dbeier", 
                              "damage_steel_dbeier_total"]

# overwrite any existing damage record file of this name
with open('damage_results_copper_pthies.csv', 'w', newline='') as csvfile:
    field_names = ["Hs_sim", "T_sim", "dir_sim", "n_sim", "damage_copper_pthies", "scaled_damage_copper_pthies", 
                   "damage_copper_pthies_total"]
    writer = csv.DictWriter(csvfile, fieldnames = field_names)
    writer.writeheader()

with open('damage_results_copper_dbeier.csv', 'w', newline='') as csvfile:
    field_names = ["Hs_sim", "T_sim", "dir_sim", "n_sim", "damage_copper_dbeier", "scaled_damage_copper_dbeier", 
                   "damage_copper_dbeier_total"]
    writer = csv.DictWriter(csvfile, fieldnames = field_names)
    writer.writeheader()
...
with open('damage_results_steel_dbeier.csv', 'w', newline='') as csvfile:
    field_names = ["Hs_sim", "T_sim", "dir_sim", "n_sim", "damage_steel_dbeier", "scaled_damage_steel_dbeier", 
                   "damage_steel_dbeier_total"]    
    writer = csv.DictWriter(csvfile, fieldnames = field_names)
    writer.writeheader()
    # because using 'with', the file is closed, don't worry
...
for  dir_Hs_T_n_tuple in dir_Hs_T_n:
    ...
    model.SaveSimulation("./simfiles/BatchSimulation-" + str(Hs_sim) + "m_" + str(T_sim) + "s_" + str(n_sim) + "n_" \
                         + str(dir_sim) + "dir" + "_" + all_sims_start_dateTime.strftime("%H-%M-%S_%d-%m-%Y") + ".sim")
    ...
    orcaflex_batch.tension_history = np.multiply(array_cable.TimeHistory("Wall tension", OrcFxAPI.SpecifiedPeriod(
    ...
    orcaflex_batch.bend_moment_history = np.multiply(array_cable.TimeHistory("Bend moment", OrcFxAPI.SpecifiedPeriod(
        model.simulationStartTime,model.simulationStopTime),OrcFxAPI.oeArcLength(0.5)), 1000)    
    ...
    orcaflex_batch.curvature_y_history = array_cable.TimeHistory("y curvature", OrcFxAPI.SpecifiedPeriod(
        model.simulationStartTime,model.simulationStopTime),OrcFxAPI.oeArcLength(0))    
    # greatest curvature at 0 metres (from observations) in rad/m (SI)
    #orcaflex_batch.x_bend_moment_history = np.multiply(array_cable.TimeHistory("x bend moment", OrcFxAPI.SpecifiedPeriod(
    #model.simulationStartTime,model.simulationStopTime),OrcFxAPI.oeArcLength(0)), 1000)    
    ...
    # greatest (negative) bend moment at 0 metres (from observations) (kNm) (*1e3 to SI - or is it - code values unlike GUI may be in SI units Nm, so * 1) 
    ...
    bend_moment = array_cable.RangeGraph("Bend moment", OrcFxAPI.SpecifiedPeriod(
        model.simulationStartTime,model.simulationStopTime)).Max
    bend_moment_df = pd.DataFrame(bend_moment)      # get maximum bend moment along cable sections/"arc lengths"
    ...
    orcaflex_batch.tension_stress_concentrator_copper_1 = orcaflex_batch.Kt1(
        orcaflex_batch.CONDUCTOR_AREA, orcaflex_batch.ARMOUR_AREA, orcaflex_batch.MODULUS_COPPER, orcaflex_batch.MODULUS_STEEL)
    ...
    orcaflex_batch.curvature_stress_concentrator_copper_1 = orcaflex_batch.Kc1(orcaflex_batch.YIELD_STRENGTH_COPPER)
    orcaflex_batch.curvature_stress_concentrator_steel_2 = orcaflex_batch.Kc2(orcaflex_batch.YIELD_STRENGTH_STEEL)
    ...
    print('Stress concentrator values \n copper: \t%f \nsteel: \t%f '%(
        orcaflex_batch.tension_stress_concentrator_copper_1,orcaflex_batch.tension_stress_concentrator_steel_2))
    ...
    orcaflex_batch.DBeier_tension_stress_copper = np.multiply(
        orcaflex_batch.tension_history,orcaflex_batch.tension_stress_concentrator_copper_1)   # Kt * T (element-wise)
    ...
    curvature_components = np.subtract(np.multiply(orcaflex_batch.curvature_x_history,math.sin(theta)), np.multiply(
        orcaflex_batch.curvature_y_history,math.cos(theta)))
    ...
    damage_steel_dbeier = orcaflex_batch.calculate_damage(orcaflex_batch.total_stress_2, 
                                                          orcaflex_batch.steel_sn, orcaflex_batch.FATIGUE_LIMIT_STEEL)
